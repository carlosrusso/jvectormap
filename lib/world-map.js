...
jvm.WorldMap.prototype = {
  ...
  createRegions: function(){
    ...
    for (key in this.mapData.paths) {
      ...
      map.container.trigger('regionCreated.jvectormap', [key, this.mapData.paths[key],region,map]);
    }
  },

  drawMarker: function(pointConfig, styleConfig, group){
    if (typeof pointConfig.renderer === 'function'){
      return pointConfig.renderer( this.canvas, pointConfig, styleConfig, group);
    } else {
      return this.canvas.addCircle({
        cx: pointConfig.x,
        cy: pointConfig.y,
        "data-index": pointConfig['data-index']
      }, styleConfig, group);
    }
  },
  createMarkers: function(markers) {
    ...
    for (i in markers) {
      ...
      if (point !== false) {
        var pointConfig ={
          "data-index": i,
          renderer: markerConfig.renderer || undefined,
          x: point.x,
          y: point.y
        }, styleConfig = jvm.$.extend(true, {}, this.params.markerStyle, {initial: markerConfig.style || {}});
        marker = this.drawMarker(pointConfig, styleConfig, this.markersGroup);
        ...
        map.container.trigger('markerCreated.jvectormap', [i, markers[i],marker,this]);
      }
    }
  },
  ...
  repositionMarkers: function() {
    ...
    for (i in this.markers) {
      ...
      if (point !== false && this.markers[i].element.setStyle) {
        this.markers[i].element.setStyle(
          typeof this.markers[i].config.renderer === 'function' ? {x: point.x, y: point.y} : {cx:point.x, cy: point.y}
        );
      } else {
        var bbox = this.markers[i].element.getBBox();
        var xfin = point.x - bbox.width;
        var yfin = point.y - (bbox.height/2);
          this.markers[i].element.applyAttr('transform','translate('+xfin+','+yfin+')')
      }
    }
  },
...
jvm.WorldMap.apiEvents = {
  ...
  onRegionCreated: 'regionCreated',
  onMarkerLabelShow: 'markerLabelShow',
  onMarkerOver: 'markerOver',
  onMarkerOut: 'markerOut',
  onMarkerClick: 'markerClick',
  onMarkerSelected: 'markerSelected',
  onMarkerCreated: 'markerCreated',
